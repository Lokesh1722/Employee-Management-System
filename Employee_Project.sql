...
SELECT EmpID, COUNT(Requirements) 'List_Of_Qualification'
FROM qualification
GROUP BY EmpID
ORDER BY List_Of_Qualification DESC
LIMIT 1;
...
SELECT FirstName, COUNT(Requirements) as 'highest_Number_Of_Qualification'
FROM employee e
...
SELECT AVG(extract(day from Date)) AS 'Avg_no_of_leaves'
FROM leaves;
